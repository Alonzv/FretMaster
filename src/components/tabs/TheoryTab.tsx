...
export default function TheoryTab() {
  ...
  // If an article is open, render it full-screen inside the content area
  if (openArticleId) {
    return (
      <div style={{ position: 'relative', height: '100%', overflow: 'hidden' }}>
        <ArticleReader articleId={openArticleId} onBack={() => setOpenArticleId(null)} />
      </div>
    )
  }
        ...
        {articles.map(article => (
          ...
          >
            ...
            <div style={{
              ...
              {article.sections.length} {isHe ? 'פרקים' : 'sections'}
            </div>
          </button>
        ))}
      </div>
