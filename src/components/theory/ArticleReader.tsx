  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: section.title ? 16 : 0 }}>
        <div style={{
          width: 30, height: 30, background: iconBg, display: 'flex',
...
function ExampleSection({ section }: { section: ArticleSection }) {
  // Split on newline so each entry becomes its own block
  ...
  return (
    <div style={{ marginBottom: 48 }}>
      ...
      {/* Each entry as its own blockquote card */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {entries.map((entry, i) => {
          // First sentence (up to ":") is the bold title
...
export default function ArticleReader({ articleId, onBack }: Props) {
  ...
  if (!article) {
    return (
      <div dir="rtl" style={{ padding: 40, color: '#1A1A2E', fontFamily: 'var(--fm-font-body)' }}>
        מאמר לא נמצא.
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {/* Progress bar — razor thin, lives at the very top */}
      ...
      >
        ...
        <div style={{
          ...
          <div style={{
            height: 2,
            background: 'linear-gradient(to left, #2B50E8 0%, #1A1A2E 40%, transparent 100%)',
            marginBottom: 52,
          }} />
        </div>
